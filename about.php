<!DOCTYPE html>
<html>
    <head>
        <title>About Us</title>
        <link rel="stylesheet" href="CSS/style.css"/>
        <link rel="shortcut icon" href="Images/Favicon/favicon.ico" type="image/ico">
        <script
          src="https://code.jquery.com/jquery-3.2.1.min.js"
          integrity="sha256-hwg4gsxgFZhOsEEamdOYGBf13FyQuiTwlAQgxVSNgt4="
          crossorigin="anonymous"></script>
        <script src="JS/script.js"></script>
        <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js" integrity="sha384-Tc5IQib027qvyjSMfHjOMaLkfuWVxZxUPnCJA7l2mCWNIpG9mGCD8wGNIcPD7Txa" crossorigin="anonymous"></script>
        <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous">
    </head>
    <body class="white">
        <div id="navbar">
            <div class="col-xs-2" id="backbtn"><span class="glyphicon glyphicon-menu-left"></span></div>
            <div class="col-xs-10" id="navbartext">About Us</div>
        </div>
        <div class="row">
            <div id="numetrologo" class="col-xs-6 col-xs-offset-3">
                LOGO
            </div>
        </div>
        <div class="row">
            <div id="logosubtext" class="col-xs-6 col-xs-offset-3">
                We always want you to<br>
                have a great time!
            </div>
        </div>
        <div class="row">
            <div id="socialmedialinksheading" class="col-xs-8 col-xs-offset-2">
                Join us on social media for all the latest<br>
                movie info and gossip!
            </div>
        </div>
        <div id="facebookbar" class="col-xs-12">
            Like us on Facebook!
        </div>
        <div id="twitterbar" class="col-xs-12">
            Follow us on Twitter!
        </div>
        <div id="instagrambar" class="col-xs-12">
            Follow us on Instagram!
        </div>
        <div id="versionnumber" class="col-xs-12">
            version No 1.0.0
        </div>
    </body>
</html>